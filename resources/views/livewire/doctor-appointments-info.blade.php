                        ...
                        <table class="table main-table special-table">
                            <thead>
                                <tr>
                                    {{--
                                        <th scope="col">
                                            <div class="top_word text-center">{{$from ? date('l', strtotime($from . ' +6 day')) : date('l', strtotime('+6 day'))}}
                    </div>
                    <div class="bottom_word text-center">
                        {{$from ? date('Y-m-d', strtotime($from . ' +6 day')) : date('Y-m-d', strtotime('+6 day'))}}</div>
                    </th>
                    <th scope="col">
                        <div class="top_word text-center">
                            {{$from ? date('l', strtotime($from . ' +5 day')) : date('l', strtotime('+5 day'))}}</div>
                        <div class="bottom_word text-center">
                            {{$from ? date('Y-m-d', strtotime($from . ' +5 day')) : date('Y-m-d', strtotime('+5 day'))}}</div>
                    </th>
                    <th scope="col">
                        <div class="top_word text-center">
                            {{$from ? date('l', strtotime($from . ' +4 day')) : date('l', strtotime('+4 day'))}}</div>
                        <div class="bottom_word text-center">
                            {{$from ? date('Y-m-d', strtotime($from . ' +4 day')) : date('Y-m-d', strtotime('+4 day'))}}</div>
                    </th>
                    <th scope="col">
                        <div class="top_word text-center">
                            {{$from ? date('l', strtotime($from . ' +3 day')) : date('l', strtotime('+3 day'))}}</div>
                        <div class="bottom_word text-center">
                            {{$from ? date('Y-m-d', strtotime($from . ' +3 day')) : date('Y-m-d', strtotime('+3 day'))}}</div>
                    </th>
                    <th scope="col">
                        <div class="top_word text-center">
                            {{$from ? date('l', strtotime($from . ' +2 day')) : date('l', strtotime('+2 day'))}}</div>
                        <div class="bottom_word text-center">
                            {{$from ? date('Y-m-d', strtotime($from . ' +2 day')) : date('Y-m-d', strtotime('+2 day'))}}</div>
                    </th>
                    <th scope="col">
                        <div class="top_word text-center">
                            {{$from ? date('l', strtotime($from . ' +1 day')) : date('l', strtotime('+1 day'))}}</div>
                        <div class="bottom_word text-center">
                            {{$from ? date('Y-m-d', strtotime($from . ' +1 day')) : date('Y-m-d', strtotime('+1 day'))}}</div>
                    </th>
                    <th scope="col">
                        <div class="top_word text-center">{{$from ? date('l', strtotime($from)) : date('l')}}</div>
                        <div class="bottom_word text-center text-center">{{$from ? date('Y-m-d', strtotime($from)) : date('Y-m-d')}}
                        </div>
                    </th>
                    --}}

                                    @php
                                        $start_of_month = \Carbon\Carbon::now()->startOfMonth()->isoFormat('Y-MM-DD');
                            ...
                            <tbody>
                                ...
                                @foreach ($times as $time)
                                    <tr class="fw-bolder">
                                        @for ($i = 0; $i < $count_days; $i++)
                                            @php$fromTime = date('H:i', strtotime($time));
                                                                                                $toTime = date('H:i', strtotime($time . '+30 minute')); // $appointment=$appointments->
                                                                                                where('appointment_status', 'cancelled')->first();
                                                                                                $appointment = $appointments
                                                                                                    ...
                                                                                                    ->where(
                                                                                                        'appointment_date',
                                                                                                        $from
                                                                                                            ? date('Y-m-d', strtotime($from . ' +' . $i . ' day'))
                                                                                                            : date(
                                                                                                                'Y-m-d',
                                                                                                                strtotime($start_of_month . '+' . $i . 'day'),
                                                                                                            ),
                                                                                                    )
                                                                                                    ->first();
                                                                                                ...
                                                                                                if ($appointment) {
                                                                                                    if ($appointment->appointment_status == 'pending') {
                                                                                                        ...
                                                                                                        $bg_class = 'bg-danger';
                                                                                                    } elseif ($appointment->appointment_status == 'transferred') {
                                                                                                        $bg_class = 'bg-dark';
                                                                                                    } else {
                                                                                                        $bg_class = 'bg-primary';
                                                                                                    }
                                            ...
                                            @if ($appointment)
                                                <td scope="row" class="text-center {{ $bg_class }}">
                                                    <div class="toltip-table">
                                                        ...
                                                        <a href="{{ route('front.appointments.edit', $appointment->id) }}"
                                                            class="btn btn-sm btn-primary mt-2 w-fit d-block mx-auto">{{ __('admin.Update') }}</a>
                                                    </div>
                                                </td>
                                            @else
