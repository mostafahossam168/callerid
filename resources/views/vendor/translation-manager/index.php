    ...
    <div class="container">
        <div class="shadow rounded border p-3">
            ...
            <div class="alert alert-success success-import" style="display:none;">
                <p>تم الانتهاء من الاستيراد والمعالجة <strong class="counter"></strong> قم بتحديث هذه الصفحة لتحديث المجموعات!</p>
            </div>
            <div class="alert alert-success success-find" style="display:none;">
                <p>انتهيت من البحث عن الترجمات</p>
            </div>
            <div class="alert alert-success success-publish" style="display:none;">
                <p>تم الانتهاء من نشر الترجمات للمجموعة '<?php echo $group ?>'!</p>
            </div>
            <div class="alert alert-success success-publish-all" style="display:none;">
                <p>تم الانتهاء من نشر الترجمات لكل المجموعة!</p>
            </div>
                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            <select name="replace" class="form-control">
                                <option value="0">إلحاق ترجمات جديدة</option>
                                <option value="1">استبدال الترجمات الموجودة</option>
                            </select>
                        </div>
                        <div class="col-sm-2">
                            <button type="submit" class="btn btn-success btn-block" data-disable-with="Loading..">استيراد المجموعات</button>
                        </div>
                    </div>
                </div>
            </form>
            <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
                <div class="form-group">
                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                    <button type="submit" class="btn btn-info" data-disable-with="Searching..">البحث عن الترجمات في الملفات</button>
                </div>
            </form>
        <?php endif; ?>
        <?php if (isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing..">نشر الترجمات</button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">رجوع</a>
            </form>
        <?php endif; ?>
        </p>
        <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
            ...
            <div class="row">
                ...
                <div class="col-12 col-md-6">
                    <div class="form-group">
                        <label>أدخل اسمًا جديدًا للمجموعة وابدأ في تعديل الترجمات</label>
                        <input type="text" class="form-control" name="new-group" />
                    </div>
                </div>
            </div>
            <div class="form-group">
                <input type="submit" class="btn btn-default" name="add-group" value="إضافة وتحرير المفاتيح" />
            </div>
        </form>
        <?php if ($group) : ?>
            <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST" role="form">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <div class="form-group">
                    <label>أضف مفاتيح جديدة لهذه المجموعة</label>
                    <textarea class="form-control" rows="3" name="keys" placeholder="أضف مفتاحًا واحدًا في كل سطر، بدون بادئة المجموعة"></textarea>
                </div>
                <div class="form-group">
                    <input type="submit" value="إضافة مفاتيح" class="btn btn-primary">
                </div>
            </form>
            <div class="row">
                <div class="col-sm-2">
                    <span class="btn btn-default enable-auto-translate-group">استخدم الترجمة التلقائية</span>
                </div>
                <div class="col-sm-2">

                <a href="/admin/generate-translation" class="btn btn-default">
                    اضافة الترجمه الناقصة
                </a>
            </div>
            ...
            <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postTranslateMissing') ?>">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label for="base-locale">اللغة الأساسية للترجمات التلقائية</label>
                        ...
                        <div class="form-group">
                            <label for="new-locale">أدخل مفتاح اللغة المستهدفة</label>
                            <input type="text" name="new-locale" class="form-control" id="new-locale" placeholder="أدخل مفتاح اللغة المستهدفة" />
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <button type="submit" class="btn btn-default btn-block" data-disable-with="Adding..">الترجمة التلقائية</button>
                        </div>
                    </div>
                </div>
            </form>
            <hr>
            <h4>المجموع: <?= $numTranslations ?>, تغير: <?= $numChanged ?></h4>
            <table class="table">
                <thead>
                    <tr>
                        <th width="15%">Key</th>
        ...
        <?php else : ?>
            <fieldset>
                <legend>اللغات المدعومة</legend>
                <p>
                اللغات المدعومة الحالية:
                </p>
...
<!--                                    </button>-->
                ...
                <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                    ...
                    <div class="form-group">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                <button type="submit" class="btn btn-default btn-block" data-disable-with="Adding..">إضافة لغة جديدة</button>
                            </div>
                        </div>
                    </div>
                </form>
            ...
            <fieldset>
                <legend>تصدير كافة الترجمات</legend>
                <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                    <button type="submit" class="btn btn-primary" data-disable-with="Publishing..">نشر الكل</button>
                </form>
            </fieldset>
