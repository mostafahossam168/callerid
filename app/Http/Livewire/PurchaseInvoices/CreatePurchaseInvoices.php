...
class CreatePurchaseInvoices extends Component
{
    public $supplier_id, $warehouse_id, $amount = 0, $items = [], $invoice_data, $tax = 0, $total = 0;
    ...
    public function save()
    {
        ...
        try {
            ...
            $voucher = Voucher::create([
                'date' => date('Y-m-d'),
                'description' => 'فاتورة مشتريات رقم ' . $invoice?->id . ' من المورد ' . $invoice->supplier?->name,
            ...
            $voucher->accounts()->createMany(
                [
                    ...
                    [
                        'voucher_id' => $voucher->id,
                        'account_id' => $invoice->warehouse?->account_id,
                        'credit' => 0,
                        'debit' => $this->amount,
                        'description' => $voucher->description,
                    ]
